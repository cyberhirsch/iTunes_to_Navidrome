...
*   [Scripts Overview](#scripts-overview)
    ...
    *   [`navidrome_playlist_tool.py` (Local/Server Playlist Management)](#navidrome_playlist_toolpy-localserver-playlist-management)
...
### `navidrome_playlist_tool.py` (Local/Server Playlist Management)
...
*   **`navidrome_playlist_tool.py`** will prompt for and save:
...
The `itunesPlaylistMigrator.py` and `navidrome_playlist_tool.py` both manage Navidrome server connection details within `config.json`. However, they use **different key names** (`server_url`, `username`, `password` vs `navidrome_url`, `navidrome_user`, `navidrome_password` respectively).
...
#### How to Use `navidrome_playlist_tool.py`

1.  **Ensure Navidrome is Running.**
2.  **Run the Script:** `python navidrome_playlist_tool.py`
...
For features that parse local M3U files (like `navidrome_playlist_tool.py`'s playlist checking), the script expects track paths within your M3U files to generally follow a `Artist/Album/Track Title.ext` structure. For example:
